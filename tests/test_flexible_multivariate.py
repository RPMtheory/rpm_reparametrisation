...
batch_shapes = [100, 13, 10]
event_shapes = 30
natural1 = torch.randn(*batch_shapes, event_shapes)
natural2_vec = torch.randn(*batch_shapes, int(event_shapes * (event_shapes + 1) / 2))
natural2_chol = vector_to_tril(natural2_vec)
natural2 = - torch.matmul(natural2_chol, natural2_chol.transpose(-1, -2))
natural2_sparse = natural2[0, :, 0].unsqueeze(0).unsqueeze(2)
natural2 = natural2_sparse.repeat(batch_shapes[0], 1, batch_shapes[2], 1, 1)

import time

start = time.time()

TPM1 = FlexibleMultivariateNormal(
     natural1,
     natural2,
     init_natural=True,
     init_cholesky=False,
...
TPM2 = FlexibleMultivariateNormal(
     natural1,
     natural2_sparse,
     init_natural=True,
     init_cholesky=False,
...
if (TPM2.mean_covariance()[0] - TPM1.mean_covariance()[0]).abs().sum() < 1e-6:
    print('Passed: Broadcasting test')
    print('Time Improvement: ' + str(np.round(time2, 3)) + 's Vs ' + str(np.round(time1, 3)) + 's')
else:
    print('Failed test')
