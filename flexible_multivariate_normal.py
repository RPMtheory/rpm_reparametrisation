...
class FlexibleMultivariateNormal:
    ...
    def __init__(self,
        ...
        if not (batch_shape_vec == batch_shape_mat):
            # Check that matrix_param can be broadcasted
            # TODO: Maybe also broadcast 1st parameter
            mismatch_dim = np.array(batch_shape_vec) != np.array(batch_shape_mat)
            if not np.all(np.array(batch_shape_mat)[np.where(mismatch_dim)] == 1):
                raise ValueError("Incompatible Event Dimensions: broadcasted dimensions should have size 1")

        if not (event_shape_mat[0] == event_shape_vec) or not (event_shape_mat[1] == event_shape_vec):
            # Check event dimension
    ...
    def get_batch_identity(self, eps=1e-6):
        ...
        Id = torch.eye(self.event_shape, device=self.device, dtype=self.dtype, requires_grad=False)
...
def cholesky_vector_to_natural2(chol_vec):
    """Returns Second natural parameter from Vectorized Negative Cholesky decomposition"""
    chol_tril = vector_to_tril(chol_vec)
    return - torch.matmul(chol_tril, chol_tril.transpose(-1, -2))


def natural2_to_cholesky_vector(natural2):
    return tril_to_vector(torch.linalg.cholesky(-natural2))
...
def log_normalizer_sum_fast(
        natural1_a,
        natural2_a,
        natural1_b,
        natural2_b
):
    """Helper for efficient broadcasting of log normalizer"""

    # Transform 1st natural parameters for matrix operations
    natural1_a = natural1_a.unsqueeze(-1)
    natural1_b = natural1_b.unsqueeze(-1)

    # Efficiently process 2nd natural parameter
    natural2 = natural2_a + natural2_b
    natural2_chol = torch.linalg.cholesky(-natural2)
    natural2_inv = -torch.cholesky_inverse(natural2_chol)
    natural2_det = 2 * torch.log(natural2_chol.diagonal(dim1=-1, dim2=-2)).sum(dim=-1)
    constant = torch.tensor(0.5 * natural2.shape[-1] * np.log(np.pi), device=natural2.device, dtype=natural2.dtype)
    halflogdet = - 0.5 * natural2_det + constant

    # mahalanobis distance with broadcasted operations
    ma = matmul(natural2_inv, natural1_a)
    mb = matmul(natural2_inv, natural1_b)
    maa = matmul(ma.transpose(-1, -2), natural1_a).squeeze(-1).squeeze(-1)
    mbb = matmul(mb.transpose(-1, -2), natural1_b).squeeze(-1).squeeze(-1)
    mab = matmul(ma.transpose(-1, -2), natural1_b).squeeze(-1).squeeze(-1)
    mahalanobis = -0.25 * (maa + 2 * mab + mbb)

    return mahalanobis + halflogdet
