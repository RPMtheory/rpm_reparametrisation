...
def rpm_save(
        ...
        filename: str,
        device: str = "cpu",
        observations: RPMData = None,
):
    """ Helper to save a RP-FA model (converts all objects to device)"""
...
def _to_device(x, device):
    ...
    elif isinstance(x, RPMData):
        y = RPMData(
            observations=[xi.to(device) for xi in x[:][0]],
            latent_true=x[:][1].to(device)
        )
    else:
...
def _dictionarize(
    ...
    dict_model = {
        key: _to_device(dict_model[key], device)
